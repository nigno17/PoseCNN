...
g++ -std=c++11 -shared -o hough_voting.so hough_voting_op.cc \
        ...
        -fPIC -lcudart -lcuda -lcublas -lopencv_imgproc -lopencv_calib3d -lopencv_core `pkg-config --cflags --libs opencv` -lgomp -lnlopt -L $CUDA_PATH/lib64 -L$TF_LIB -ltensorflow_framework -lpthread -Wl,-z,defs
...
g++ -std=c++11 -Wl,-z,defs  -shared -o roi_pooling.so roi_pooling_op.cc \
